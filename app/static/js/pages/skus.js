...
export function mountSkusPage(openSku) {
  ...
  root.innerHTML = `<div class="toolbar card compact"><input id="sku-search" type="search" placeholder="搜索商品名、SKU、商品 ID"><select id="severity-filter" class="control"><option value="all">全部状态</option><option value="high">严重</option><option value="medium">关注</option><option value="low">轻微</option><option value="healthy">健康</option><option value="unrun">未诊断</option></select><button id="run-shop-diagnosis" class="button primary">批量诊断</button></div><article class="card"><div class="card-heading"><div><p class="section-kicker">SKU WORKBENCH</p><h2>SKU 经营状态</h2></div><span id="sku-count" class="muted">0 个 SKU</span></div><div class="table-wrap"><table class="data-table"><thead><tr><th>商品 / SKU</th><th>销量</th><th>GMV</th><th>库存</th><th>健康度</th><th>第一优先问题</th><th></th></tr></thead><tbody id="sku-table-body"></tbody></table></div><div id="sku-empty" class="empty-state hidden"></div></article>`;
  ...
  $("#run-shop-diagnosis").addEventListener("click", async (event) => {
    ...
    try {
      const result = await api(`/api/diagnosis/shops/${state.selectedShopId}/run`, { method: "POST" });
      const counts = result.severity_counts || {};
      toast(`诊断完成：${result.success} 个 SKU · 严重 ${counts.high || 0} · 关注 ${counts.medium || 0}`, "success");
      await loadSkus(openSku);
...
export async function loadSkus(openSku) {
  ...
  try {
    ...
    body.innerHTML = data.items.map((item) => {
      ...
      const sev = diag?.severity || "unrun";
      const issue = diag?.main_issue || (diag ? `${diag.issue_count || 0} 个问题` : "尚未诊断");
      return `<tr><td><div class="product-line">${item.image_url ? `<img class="product-thumb" src="${escapeHtml(item.image_url)}" alt="">` : '<span class="product-thumb"></span>'}<span><strong>${escapeHtml(item.product_title)}</strong><small>${escapeHtml(item.sku_name)} · ${escapeHtml(item.platform_sku_id)}</small></span></div></td><td>${metric.sales_qty ?? 0}</td><td>${money(metric.gmv)}</td><td>${item.stock ?? metric.stock ?? "—"}</td><td><span class="status-pill ${sev}">${severityLabel(sev)}</span> ${diag?.health_score ?? "—"}</td><td>${escapeHtml(issue)}</td><td><button class="button ghost" data-open-sku="${item.id}">查看诊断</button></td></tr>`;
    }).join("");
