...
趋势与同商品 SKU 比较是独立增强层，详见 `docs/trend-analysis.md`。趋势层目前用于解释和导航，不直接修改单日诊断 `priority_score`。
...
- 最近 7 日 vs 前 7 日。
- 最近 30 个真实数据日趋势展示。
- 同商品 SKU 最近 7 日 GMV 排名与占比。
- SKU GMV 集中度 HHI。
- 当前 issue code 连续诊断天数。
- 店铺级下滑 SKU 概览。

趋势增强层的详细公式、缺失值规则和 HHI 分档以 `docs/trend-analysis.md` 为准。
...
在没有单独修改诊断算法前，趋势结果不得直接改写 `severity / health_score / priority_score`。
